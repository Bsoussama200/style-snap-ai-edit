...
serve(async (req) => {
  ...
  try {
    ...
    if (mode === 'motion') {
      let systemPrompt = 'You craft short, production-ready prompts to animate a still image into a simple 5-second portrait 9:16 video with tasteful camera motion (e.g., slow dolly-in, parallax, slight rack focus) and subtle effects (e.g., soft glow, vignette). Return JSON strictly as {"prompt": string} (<= 220 chars). No extra text.';
      let userText = 'Generate a short motion/effects prompt for 5 sec, 720x1280, 9:16.';
      
      if (optionsContext) {
        systemPrompt = 'You craft short, production-ready prompts to animate a still image into a simple 5-second portrait 9:16 video. Follow the specific requirements provided by the user for camera movement, effects, and product usage. Return JSON strictly as {"prompt": string} (<= 220 chars). No extra text.';
        userText = `${optionsContext} Generate a short motion/effects prompt for 5 sec, 720x1280, 9:16.`;
      }
